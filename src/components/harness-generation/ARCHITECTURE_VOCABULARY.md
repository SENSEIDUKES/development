...
| CAPA Schema | `CAPA_SCHEMA` in `shared/skills.ts` | The single ordered slot registry (Author, Pacing, Fate, Continuity, Style, Accessibility, Translation) with each slot's responsibility. Loadout freezing and CAPA assembly both iterate it, so its order is the assembled order. Fate is the one mode-managed slot (`managedBy: 'fate-mode'`): loadout freezing fills it from the chapter's frozen Fate mode, with SEN's Fate Survival skill (`shared/fateSurvivalSkill.ts`) on every Fate Survival call and nothing in Regular Reader mode, and it is never equipped by hand. Media is not a CAPA slot. |
