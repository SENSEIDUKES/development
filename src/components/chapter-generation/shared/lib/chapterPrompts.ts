...
You MUST output strictly the chapter text structured as NDJSON (Newline Delimited JSON). Start it with ---CHAPTER_BLOCKS--- on a new line. Each paragraph of your chapter should be a single JSON object on one line containing "type" (either "paragraph" or "dialogue"), "text" (the paragraph content), and optional "metadata" for narrative coordination. Do not output block IDs; the application assigns stable chapter-position IDs after validation.
...
World Cues represent deliberate audible narrative events, never entities. Add an "audioMoments" array inside a block's "metadata" only when that block contains an explicit audible action such as a beast roaring, a blade leaving its sheath, an artifact activating, an ability firing, a faction chanting, or a location's bell or gong sounding. Attach the cue to the exact, case-sensitive action phrase that produces the sound. Do not annotate an entity name, mention, reveal, discovery, or presence merely because a matching creature, weapon, faction, artifact, or location exists.
Each model-authored item must have this shape: { "triggerPhrase": "drew the blade from its sheath", "occurrenceIndex": 0, "sourceCategory": "weapons", "variation": "unsheathe", "semanticTags": ["sword", "draw", "metal"], "relatedEntity": { "name": "Ashen Sword", "type": "artifact" } }. occurrenceIndex is zero-based among exact matches within that same block and may be omitted only for the first occurrence. sourceCategory must be exactly "beasts", "weapons", "artifacts", "locations", or "factions". relatedEntity is optional and may contain only name and type. Do not output blockId; the application attaches its code-owned block reference during validation.
Never output a URL, URI, filename, file path, asset ID, catalog ID or entry, provider field, voice key, voice ID, or any other technical media selector. The application validates semantic intent and resolves approved Library Cues later. Dialogue voice assignment is separate from World Cues; never put spoken dialogue in audioMoments. Atmosphere loops and System Panel sounds remain under their existing metadata and must not be represented as World Cues.
Creature species and other Bestiary entries remain informational Codex data. They never receive an audio marker from a name or species encounter. A creature can receive a World Cue only when the prose contains the exact audible action phrase, such as "opened its jaws and roared."

CREATURE EVENT METADATA (NOT AUDIO):
You can include a "beastEvent" object inside the block "metadata" when encountering significant beast moments (reveals, major strikes, deaths, power surges). A beastEvent needs a "type" ("reveal", "power-up", "technique", "injury", "turning-point", "death", "breakthrough") and a "profile" (containing size, bodyType, element, movement, intelligence, threatTier, signatureSound matching the predefined schema). Use this sparingly and only on significant narrative beats. beastEvent supports creature presentation and continuity only; it must never create or imply a World Cue.
For any Celestial Library system moment (in ANY genre, per the CELESTIAL LIBRARY SYSTEM PANELS directive above), you MUST include a "system" object on the block (parallel to "metadata") to render the appropriate System presentation. The block's "text" field stores the concise primary system notice or in-prose message (e.g. "[The Heavenly Dao acknowledges your ascension. The Ninth Meridian has marked you.]"), while the "system" object stores structured metadata: "kind" (one of "system_prompt", "fate_system_prompt"), a REQUIRED "promptType" string (one of "neutral", "codex_update", "friendly_scan", "enemy_scan", "warning", "critical_danger", "progression", "breakthrough", "reward", "romance", "karmic_bond", "mystery", "fate_event", "corruption", "death_event", "quest_update", "choice_consequence", "system_error"), and a string "title". Regular System Prompts ("system_prompt") are the universal category. When relevant, preserve structured Card data with "badge": {"label": string, "value": string}, "changes": [{"direction": "gain"|"loss", "label": string, "tone": "positive"|"uncertain"|"warning"|"negative" (optional)}], optional "rows", and optional "rarity". ALWAYS set "promptType" — it determines semantic color through SEN Color Codes; pick the closest true meaning rather than guessing from presentation. Use this structured object instead of plain text brackets like [System Alert].
PRESENTATION FAMILY REQUIREMENT: Every new "system_prompt" MUST declare "presentation" as exactly "narrative", "mechanical", or "world_notice". "presentation" selects Reader layout only; "promptType" remains REQUIRED and selects semantic meaning and color only, never layout. Use "narrative" for concise System notifications. Use "mechanical" for LitRPG/stat displays. Every new Mechanical System Prompt MUST include a non-empty "status" object: use its "level", "bars" ({"label", numeric "value", positive numeric "max", optional "display", "tone": "health"|"spirit"|"progress"}), "stats" ({"label", "value", optional numeric "delta"}), "effects" ({"name", optional "detail", optional "value", optional "tone": "positive"|"negative"}), and/or "abilities" ({"name", optional "detail"}) to express the current status-card data. Do not substitute legacy "rows" for this status object in a new chapter; rows-only Mechanical data is reserved for reading older saved chapters. Optional "rarity" remains supported when it is meaningful. Use "world_notice" only for information the character physically sees, and include "worldNotice": {"entries": [{"title": string, "body": string (optional), "details": [{"label": string, "value": string}] (optional)}]}. One entry renders a single notice; multiple entries render one board. Give World Notices a direct document title such as "GUILD BOUNTY", "WANTED NOTICE", or "MISSION BRIEF". Optional "flavor" is secondary issuer or world context. World Notice data must never contain controls, choices, links, or actions; its block "text" remains the concise natural-language narration/TTS source.
If the chapter resolves a major Fate Deadline or Doom Deadline, you MUST emit a "fate_system_prompt" system block. For "fate_system_prompt", you MUST also include a "fateResult" object with fields "outcome" (must be exactly 'FATE AVERTED' or 'FATE SCARRED' or 'DOOM MANIFESTED'), "timelineScar", "permanentCosts" (array of strings), "newStoryState", "newActiveStats" (array of strings), and "genreShift". This introduces permanent consequences or "Fate scars". Fate System Prompts never receive "presentation" or "worldNotice" data. Regular "system_prompt" blocks must never include a fateResult.
For each block, list all notable Codex entities referenced in the 'entities' array inside "metadata". Each entity must have the shape: { "name": string, "type": "character"|"artifact"|"location"|"creature"|"faction", "mention": "reveal"|"reference" }. Set mention to "reveal" only for a first dramatic appearance, otherwise use "reference". Human Portraits, Non-Human Portraits, Artifacts, and Locations use this existing reveal path when presented visually; do not ask for their Codex IDs or image URLs. Important named, bonded, intelligent, or recurring non-human individuals are "character" entities with a Non-Human Portrait, not species portraits. Creature species remain informational Bestiary records and never receive generated species portraits or Codex Cards. Factions remain informational Codex records and do not receive visual Codex Cards. System and Fate moments use the structured "system" object and System Panels only.
Additionally, emit a per-scene 'music' object inside "metadata" when the scene's backing soundtrack can be described or changes: { "mood": "war"|"duel"|"serenity"|"romance"|"dread"|"mystery"|"triumph"|"tribulation"|"travel"|"tragedy"|"fighting"|"adventure"|"ambient"|"boss-fight"|"tension"|"sad"|"mystical"|"excitement"|"tired"|"horror", "region": "chinese"|"japanese"|"korean"|"western" (optional), "intensity": number (optional, 0 to 1) }.
...
Example:
---CHAPTER_BLOCKS---
{"type": "paragraph", "text": "Rain crawled down the black stones as Kael climbed higher into the mountain pass...", "metadata": {"mode": "narration", "sceneType": "travel", "environment": ["mountain", "rain", "night"], "atmosphereCategory": "rain", "atmosphereTags": ["mountain-pass", "steady-rain", "night"], "motion": "walking", "emotion": "determined", "intensity": 0.35, "tension": 0.25, "danger": 0.15, "mysticism": 0.4, "audioSignature": "rainy-mountain-walk", "entities": [{"name": "Kael", "type": "character", "mention": "reference"}], "music": {"mood": "travel", "region": "western", "intensity": 0.3}}}
{"type": "dialogue", "text": "\\"Who dares disturb my slumber?\\" Overseer Chen bellowed.", "metadata": {"mode": "dialogue", "speakerName": "Overseer Chen", "speakerRole": "villain", "emotion": "cruel", "intensity": 0.85, "tension": 0.9, "entities": [{"name": "Overseer Chen", "type": "character", "mention": "reveal"}]}}
{"type": "paragraph", "text": "The Thunder Roc emerged and loosed a shriek that split the rain.", "metadata": {"mode": "narration", "tension": 0.9, "entities": [{"name": "Thunder Roc", "type": "creature", "mention": "reveal"}], "audioMoments": [{"triggerPhrase": "loosed a shriek that split the rain", "occurrenceIndex": 0, "sourceCategory": "beasts", "variation": "screech", "semanticTags": ["screech", "high-pitched", "creature"], "relatedEntity": {"name": "Thunder Roc", "type": "creature"}}], "beastEvent": {"type": "reveal", "profile": {"size": "giant", "bodyType": "bird", "element": "lightning", "movement": "flying", "intelligence": "ancient", "threatTier": "mythic", "signatureSound": "screech"}}}}
{"type": "system", "text": "[The Heavenly Dao acknowledges your ascension. The Ninth Meridian has marked you.]", "system": {"kind": "system_prompt", "presentation": "mechanical", "promptType": "breakthrough", "title": "Breakthrough Achieved", "rarity": "Mythic", "status": {"level": "Core Formation", "bars": [{"label": "Qi", "value": 72, "max": 100, "display": "72 / 100", "tone": "spirit"}, {"label": "Progress", "value": 65, "max": 100, "tone": "progress"}], "stats": [{"label": "Meridian", "value": "Ninth", "delta": 1}], "effects": [{"name": "Heavenly Mark", "value": "Active", "tone": "positive"}], "abilities": [{"name": "Nine Meridian Method", "detail": "Stabilizes the newly opened path."}]}}`,
