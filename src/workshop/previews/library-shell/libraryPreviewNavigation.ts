...
export function readLibraryPreviewLocation(fallbackState = 'linked'): LibraryLocation {
  ...
  const screen = query.get('screen') ?? (query.get('source') === 'cultivator-cave' || state === 'profile' ? 'profile' : state === 'reader' ? 'reader' : state === 'create' ? 'creator-space' : state === 'sects' ? 'sects' : state === 'tiers' ? 'pricing' : 'home');
