...
The identity column is the only flexible item; the controls always take their intrinsic width. It keeps a floor wide enough to read the Library plaque — 10rem, 12rem from `sm`, and 11rem from `lg` where the status label and secondary actions arrive — sized so a 320px phone still fits the identity beside the controls. Letting the action group shrink instead is what put the primary action's 44px button outside its own wrapper and on top of the overflow trigger. Action labels ellipsize, and every action keeps its accessible name and tooltip.

The overflow menu paints on an opaque ground, not glass: it floats over the workspace, and a translucent surface let the page's headings and body text read straight through the menu items. Safe areas are applied by the canonical header at its own edge, and its gutters absorb a landscape notch. Reduced-motion CSS disables decorative transitions/animations in the family; Main Library uses MotionConfig's user preference. DAO adds a named modal, focus lock/return, Escape, body-scroll lock and touch-sized controls to its Development fork.
