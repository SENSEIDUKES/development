...
- **2026-08-18:** Refined the Development `CodexCard` Manifest seal. Color: the fixed cyan `#04ACFF` / violet `#7C5CFF` pair is gone — the seal button now derives `--codex-seal-light` / `--codex-seal-deep` as lighter (70% + white) and darker (78% + black) `color-mix` tones of the entity's resolved accent (neutral Library token fallback), so a special Location seal burns gold and a Portrait seal keeps its portrait color; the dragon twin copies, conic aura, drop-shadow glow, focus ring, star, and Manifesting state all read from those tones (the ambience motes were already accent-driven). Copy: the "Awaken Portrait" caption is retired — the core holds only the star and the Manifest label, since the type eyebrow beneath already names what is being manifested. Silhouette: the shared `LibraryDragonCycleIcon` tail now narrows gradually into a clean curved point rising into the head's open jaws instead of ending in a blunt stub. Name, description, card dimensions, manifestation behavior, responsiveness, and reduced-motion handling are unchanged.
