...
- **2026-08-18:** Matched the highlighted-term `CodexHovercard` seal to the refined `CodexCard`: the hard-coded cyan/violet is replaced by the entity accent through the same `--codex-seal-light` / `--codex-seal-deep` `color-mix` tones set on the seal button (neutral Library token fallback), covering the dragon twins, conic aura, drop-shadow glow, focus ring, star, and Manifesting state; the "Awaken Portrait" caption is removed from the core, leaving the star and the Manifest label. Docking, desktop placement, focus/Escape handling, trigger theming, the Manifest backdrop, and the local manifestation flow are unchanged.
