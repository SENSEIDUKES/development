import type { IdentityMode, LibraryPrincipal } from './types';
...
export interface PrincipalResolverOptions {
   ...
   */
  verifyIdToken?: IdentityVerifier;
}

export type PrincipalResolver = (request: IdentityRequest) => Promise<LibraryPrincipal | null>;
...
export const developmentIdentityToken = (uid: string) => `${DEVELOPMENT_TOKEN_PREFIX}${uid}`;

const bearerToken = (request: IdentityRequest): string | undefined => {
 ...
 */
export function createPrincipalResolver(options: PrincipalResolverOptions): PrincipalResolver {
  if (options.mode === 'production' && !options.verifyIdToken) {
    throw new Error('Library production identity mode requires a token verifier.');
  }
