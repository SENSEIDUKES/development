...
- **2026-08-25:** Split the two regular System Prompt controls along their presentations. The Narrative Notification loses its Expanded Info pop-out entirely: the retired Codex-shaped `expanded` payload, its `SystemPromptExpanded*` Reader-only types, and the viewport-locked event report overlay are gone, the orb returns to an inert emblem (`SystemOrbEmblem`, now its own module shared with the mechanical card), and the compact card keeps every piece of its existing information plus the small bottom chevron that reveals or hides the TTS summary. Structured Mechanical keeps the Expanded Info action, and it now opens the new modern holographic stat panel (`SystemStatusPanel`) — a portaled `role="dialog" aria-modal="true"` status screen at reading scale with corner brackets, the STATUS headline, class line, classification, LEVEL pill, semantic HP/QI/EXP meters, the full two-column stat grid with signed deltas, active effects, abilities, and any legacy rows. The in-flow mechanical card is now the vitals preview (headline, level pill, meters, legacy rows) and shows the orb only when the panel holds more than it does. Focus trap, scroll lock, Escape/close/backdrop dismissal, nested Codex hovercard Escape deferral, focus return to the orb, and the `data-reader-narration="excluded"` boundary carry over unchanged from the retired overlay; `SystemConsequenceRow` drops its now-unused `expanded` variant. World Notices, Fate results, reduced-motion behavior, the semantic Color Code registry, and the locked Reference are unchanged.
...
- `development/SystemOrbEmblem.tsx` → new `src/components/SystemOrbEmblem.tsx` (the shared System emblem: inert on Narrative, the Expanded Info action on Mechanical)
