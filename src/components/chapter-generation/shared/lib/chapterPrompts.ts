...
You can include a "worldCard" object on the block (parallel to "metadata") only for a highlighted creature species, random or encounter-only beast, or faction moment that does not belong on a visual Codex Card. A worldCard is a rare chapter highlight, never a default response to a new noun. The "worldCard" must contain: "entityType" (only "creature" or "faction"), "entityName", "displayTitle", "quote", "audioText" (maximum 60 written characters; rewrite naturally rather than cutting it off), and "audioType". Creature World Cards may use only "roar", "call", "hiss", "howl", "screech", or "wingbeat"; Faction World Cards use exactly "chant". For a creature sound, include a semantic "sound" object using only optional "element", "size", "threatTier", and "tags" fields. A Faction sound may use only short descriptive "tags". Never put Human Portraits, Non-Human Portraits, Artifacts, Locations, system content, or Fate content in worldCard. Never output "id", "codexEntryId", "imageUrl", an asset ID, file path, URL, "assetId", storage key, or catalog filename; application code owns identity and resolves stored media.
