...
export const HARNESS_RESPONSE_CONTRACT = [
  ...
  'The packet arrives as ordered sections: Current Story Information, Destined Ending and Hard Pins, Active Arc Goal, Fate Pressure Rhythm Direction, optional Fate Survival context, Previously On, and Current Canonical State. Each fact appears once, in its section. The Destined Ending is the novel\'s fixed destination; Hard Pins are the author\'s absolute story-wide intentions and hold for the entire story. The Destined Ending, the Hard Pins, the Fate Pressure, and the Arc Plan are author-owned: never rewrite, replace, weaken, or contradict them, and never return them as fields. The story itself travels toward the Destined Ending; when the Active Arc Goal section marks the final arc, its goals carry the story to that ending, and writing the ending into the prose then is the intended outcome, not a violation.',
...
const presentArc = (arc: NonNullable<StoryInformationPacket['arc']>) => ({
  ...
  ...(arc.plannedArcCount ? { plannedArcCount: arc.plannedArcCount, finalArc: Boolean(arc.finalArc) } : {}),
...
export const presentStoryInformationPacketSections = (packet: StoryInformationPacket): PresentedPacketSection[] => [
  ...
  { section: 'storyDirection', text: ['DESTINED ENDING AND HARD PINS (author-owned; never rewrite them. The story travels toward the Destined Ending and reaches it in its final arc)', JSON.stringify({ destinedEnding: packet.storyDirection.destinedEnding, hardPins: validateHardPinInputs(packet.storyDirection.hardPins.map(text => ({ text }))).map(pin => pin.text) }, null, 2)].join('\n') },
