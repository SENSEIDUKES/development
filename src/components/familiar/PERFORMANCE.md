...
| Drag direction was measured from the original pointer-down position, so a left/right reversal stayed on the stale animation until the pointer crossed its starting point. | Resolve direction from the newest pointer sample, commit only true direction changes inside the input event, retain the current loop through vertical-only samples, and skip repeated direction updates. Position remains batched to one update per display frame. | A regression reverses direction before crossing the drag origin and requires the matching animation immediately. |
