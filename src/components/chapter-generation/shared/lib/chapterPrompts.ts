...
For any Celestial Library system moment (in ANY genre, per the CELESTIAL LIBRARY SYSTEM PANELS directive above), you MUST include a "system" object on the block (parallel to "metadata") to render the appropriate System presentation. The block's "text" field stores the concise primary system notice or in-prose message (e.g. "[The Heavenly Dao acknowledges your ascension. The Ninth Meridian has marked you.]"), while the "system" object stores structured metadata: "kind" (one of "system_prompt", "fate_system_prompt"), a REQUIRED "promptType" string (one of "neutral", "codex_update", "friendly_scan", "enemy_scan", "warning", "critical_danger", "progression", "breakthrough", "reward", "romance", "karmic_bond", "mystery", "fate_event", "corruption", "death_event", "quest_update", "choice_consequence", "system_error"), and a string "title". Regular System Prompts ("system_prompt") are the universal category: the normal/default output is a concise literary title and message; optional "rows" (each with "label" and "value" strings) and optional "rarity" should only be included when the story genuinely requires structured mechanical detail. ALWAYS set "promptType" — it determines the panel color in the UI; pick the closest semantic category rather than omitting it. Use this structured object instead of plain text brackets like [System Alert].
PRESENTATION FAMILY REQUIREMENT: This supersedes the regular System Prompt guidance above. Every new "system_prompt" MUST declare "presentation" as exactly "narrative", "mechanical", or "world_notice". "presentation" selects Reader layout only; "promptType" remains REQUIRED and selects semantic meaning and color only, never layout. Use "narrative" for concise System notifications. Use "mechanical" for LitRPG/stat displays and include optional "rows" (each with "label" and "value") plus optional "rarity" only when the story truly needs them. Use "world_notice" only for information the character physically sees, and include "worldNotice": {"entries": [{"title": string, "body": string (optional), "details": [{"label": string, "value": string}] (optional)}]}. One entry renders a single notice; multiple entries render one board. Give World Notices a direct document title such as "GUILD BOUNTY", "WANTED NOTICE", or "MISSION BRIEF". Optional "flavor" is secondary issuer or world context. World Notice data must never contain controls, choices, links, or actions; its block "text" remains the concise natural-language narration/TTS source.
