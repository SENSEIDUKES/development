...
describe('Harness context priorities', () => {
  ...
  it('prioritizes the latest appended correction at equal timestamps and explains oversized omissions', () => {
    const { state, story, foundation, correction } = fixture();
    ...
    expect(tight.canonicalContext!.corrections.map(item => item.id)).toEqual(['newer']);
    expect(tight.selectionAudit!.omitted.find(item => item.sourceKind === 'correction')!.reason).toContain('newer corrections');
    ...
    expect(tiny.committedChapters).toEqual([]);
  });
