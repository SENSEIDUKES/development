...
You can include a "worldCard" object on the block (parallel to "metadata") for a rare, highlighted World Card or established audio moment. It is never a default response to a new noun. The "worldCard" must contain: "entityType" (only "character"|"creature"|"artifact"|"location"|"faction"), "entityName", "displayTitle", "quote", "audioText" (maximum 60 written characters; rewrite naturally rather than cutting it off), and "audioType". Use only the role allowed for the entity: character "tts_line"; creature "roar", "call", "hiss", "howl", "screech", or "wingbeat"; artifact "unsheathe", "metallic_ring", "reload", "activation_hum", "resonance", "awakening", "pulse", or "magical_activation"; location exactly "signature"; faction exactly "chant". For every non-character sound, include a semantic "sound" object using only optional "element", "size", "threatTier", "weaponType", "artifactCategory", "assetFamily" (artifact only: exactly "weapon" or "relic"), and "tags" fields. For an artifact with "magical_activation", always set "assetFamily" so weapon and relic sounds remain separate. Never emit a worldCard on a block whose metadata.entities includes a visual Codex reveal: a reveal for a Human Portrait, Non-Human Portrait, Artifact, or Location. Put a World Card highlight or its audio moment on a different supporting block instead. System content and Fate content never use worldCard; they use System Panels. Never output "id", "codexEntryId", "imageUrl", an asset ID, file path, URL, "assetId", storage key, or catalog filename; application code owns identity and resolves stored media.
