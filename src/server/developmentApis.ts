...
export const generationApis = (
  ...
  const economy = createDevelopmentEconomy(environment);
  const configure = (server: { middlewares: { use: (handler: (
    ...
    server.middlewares.use(async (request, response, next) => {
      ...
      if (pathname === '/api/library-economy') {
        try {
          ...
          const result = await economy.handle(capability, { method: request.method, body, headers: request.headers });
