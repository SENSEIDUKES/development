...
export function LibraryShellWorkspace() {
  ...
  const render = (variant: 'reference' | 'development') => {
    ...
    const src = `/library-shell.html?source=${source}&state=${frameState}&variant=${variant}`;
  ...
  return <FeatureWorkspace entry={workshopEntries.find(entry => entry.id === 'library-shell')!}
    ...
    workshopControls={{ description: 'Integrated Development workspaces and Main Library header. Locked references remain available for comparison.', sections: [{ id: 'pages', content: <div className="flex flex-wrap gap-4">
