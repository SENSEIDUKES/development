...
export const GlobalHeader: React.FC = () => {
  ...
  const hasQiCharge = qiCharge > 0;
  const { open: isHubOpen, setOpen: setIsHubOpen, rootRef: hubRef, triggerRef, panelRef, id: hubId } = useHeaderDisclosure();

  useEffect(() => {
    if (!isHolding && !hasQiCharge) return;
    const intervalId = setInterval(() => {
      setQiCharge((prev) => isHolding ? Math.min(prev + 3, 100) : Math.max(prev - 6, 0));
    }, 20);
    return () => clearInterval(intervalId);
  }, [isHolding, hasQiCharge]);
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
          ...
          <div className="relative" ref={hubRef}>
            <button
              ...
              title="Command Hub"
              aria-label="Command Hub" ref={triggerRef} aria-expanded={isHubOpen} aria-controls={hubId}
            >
            ...
            <AnimatePresence>
              {isHubOpen && (
                <motion.div
                  ref={panelRef} id={hubId} onClickCapture={event => {
                    if ((event.target as Element).closest('button:not(:disabled)')) triggerRef.current?.focus();
                  }}
                  initial={{ opacity: 0, y: 15, scale: 0.95 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: 15, scale: 0.95 }}
                  transition={{ duration: 0.2, ease: "easeOut" }}
                  className="header-global-panel absolute right-0 mt-3 w-72 origin-top-right z-50 rounded-xl bg-black/95 border border-portal/30 backdrop-blur-2xl shadow-[0_10px_40px_rgba(0,0,0,0.95),0_0_20px_rgba(4,172,255,0.15)] overflow-hidden text-[#dfd8cf]"
                >
