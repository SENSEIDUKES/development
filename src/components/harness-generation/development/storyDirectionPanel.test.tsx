...
describe('HARNESS story-direction sources', () => {
  it('shows the Active Arc Goal from the Arc Plan authority, the Destined Ending, Fate Pressure, rhythm, suggestions, recaps, and the Mission Reminder', async () => {
    ...
    const goal = container.querySelector('[data-testid="harness-active-arc-goal"]')!;
    ...
    expect(goal.textContent).toContain('Next: Chapter 2');
    ...
    await act(async () => button('Open complete Arc Plan').click());
    ...
    expect(fate.textContent).toContain('Conflict · favored');
    expect(fate.textContent).toContain('A rival disciple challenges Yi Chen.');
    expect(fate.textContent).toContain('The archives reveal the founder’s oath.');
