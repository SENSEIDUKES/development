...
- **2026-08-25:** Moved the Expanded Info action from Narrative to Structured Mechanical. Narrative Notifications (Cultivation Breakthrough, Broken Promise, Target Scan) no longer expand at all — the Codex-style pop-out event report and its three local breakdown fixtures are retired, the orb rests as an inert emblem, and each compact card keeps its existing information with the small bottom chevron as its only control. The Structured Mechanical example keeps its orb as the Expanded Info action, which now opens the modern holographic stat panel: a viewport-locked status screen carrying the STATUS headline, class line, classification, LEVEL pill, semantic HP/QI/EXP meters, the full two-column stat grid with signed deltas, the active effect, and the ability. Its in-flow card becomes the vitals preview (headline, level pill, meters), and the TTS summary stays collapsed behind the centered chevron on both surfaces. Focus trap, scroll lock, Escape/close/backdrop dismissal, and focus return to the orb carry over from the retired overlay. World Notice, Fate, preset data, and the locked Reference are unchanged.
...
- every current `SystemBlock` kind, explicit Narrative Notification and LitRPG/Mechanical Display examples, single-notice and multi-entry World Notice fixtures, the compact-default/expanded disclosure states for the Structured Mechanical status screen, and all existing Fate outcomes;
...
- Fixtures are static local objects, including the Structured Mechanical status screen and the local System Prompt character entries, plus real Library test images under `/public/card-workshop/test-images`.
