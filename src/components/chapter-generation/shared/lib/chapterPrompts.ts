/**
 * Adapted port of the `chapter` section of `PROMPTS` from Light-Novels
 * `src/server/prompts.ts` (last compared with `main` on 2026-08-09). DEV now
 * keeps technical block identity code-owned while preserving the creative
 * and presentation instructions from that source. The other PROMPTS
...
You MUST output strictly the chapter text structured as NDJSON (Newline Delimited JSON). Start it with ---CHAPTER_BLOCKS--- on a new line. Each paragraph of your chapter must be a single JSON object on one line containing non-empty "text". It may optionally include "type" ("paragraph" or "dialogue") and structured presentation enrichment such as "metadata", "system", or "worldCard". Do not output block IDs, chapter numbers, word counts, timestamps, run identity, or other technical fields; code supplies them.
For dialogue blocks, optional "metadata" may describe "speakerName" (the name of the character speaking), "mode": "dialogue", and "speakerRole" (e.g. villain, main_character, face_slap, friend). DO NOT output direct voice IDs.
...
You can include a "worldCard" object on the block (parallel to "metadata") only when a named character, creature, artifact, faction, or major location makes a grand first appearance AND it has clear lasting narrative weight (major/central role plus at least two of recurrence, ownership, plot relevance, emotional significance, power significance, or future relevance). A worldCard is a rare visual event, never a default response to a new noun. Do not create one for temporary rooms, ordinary weapons, incidental shopkeepers/guards, passing disciples, or one-scene objects. When included, the "worldCard" should contain: "entityType" ("character"|"creature"|"artifact"|"location"|"faction"|"system"|"fate_event"), "entityName" (the name of the entity), "displayTitle" (e.g., "Mei Lian — The Crimson Lotus Disciple"), "quote" (a badass line or lore snippet), "audioText" (the dialogue text to synthesize or sound description; maximum 60 written characters. If the text or thought exceeds this, naturally rewrite it to preserve its main meaning and tone; never cut it off mid-word or mid-sentence), and "audioType". Use only the role allowed for the entity: character "tts_line"; creature "roar", "call", "hiss", "howl", "screech", or "wingbeat"; artifact "unsheathe", "reload", "activation_hum", "resonance", "awakening", "pulse", or "magical_activation"; location exactly "signature"; faction exactly "chant"; system or fate_event "chime". Never use location "ambience" or faction "horn", "bell", or "ceremony". For every non-character sound, include a semantic "sound" object using only these optional fields: "element", "size", "threatTier", "weaponType", "artifactCategory", "assetFamily" (artifact only: exactly "weapon" or "relic"), and "tags" (short descriptive strings). For an artifact with "magical_activation", set "assetFamily" so weapon and relic sounds remain separate. Never output an asset ID, file path, URL, "assetId", or any catalog filename; matching is handled by the reader.
For a Celestial Library system moment (in ANY genre, per the CELESTIAL LIBRARY SYSTEM PANELS directive above), you may include a "system" object on the block (parallel to "metadata") to render a holographic status panel. When present, a valid "system" object should contain "kind" (one of "status", "skill_acquired", "level_up", "quest", "appraisal", "fate_result"), a "promptType" string (one of "neutral", "codex_update", "friendly_scan", "enemy_scan", "warning", "critical_danger", "progression", "breakthrough", "reward", "romance", "karmic_bond", "mystery", "fate_event", "corruption", "death_event", "quest_update", "choice_consequence", "system_error"), a string "title", an optional array of "rows" (each with "label" and "value" strings), and an optional string "rarity". Prefer a promptType because it determines the panel color in the UI. Use this structured object instead of plain text brackets like [System Alert].
If the chapter resolves a major Fate Deadline or Doom Deadline, you may enrich its prose with a "fate_result" system object. When included, its "fateResult" object should use "outcome" (exactly 'FATE AVERTED', 'FATE SCARRED', or 'DOOM MANIFESTED'), "timelineScar", "permanentCosts" (array of strings), "newStoryState", "newActiveStats" (array of strings), and "genreShift". This presentation enrichment can describe permanent consequences or "Fate scars".
When useful, list notable Codex entities referenced in the optional 'entities' array inside "metadata". Each entity in the array must have the shape: { "name": string, "type": "character"|"artifact"|"location"|"beast"|"faction", "mention": "reveal"|"reference" }. Set mention to "reveal" ONLY for a first dramatic appearance of the entity in the story, otherwise use "reference".
You may additionally emit a per-scene 'music' object inside "metadata" when the scene's backing soundtrack can be described or changes: { "mood": "war"|"duel"|"serenity"|"romance"|"dread"|"mystery"|"triumph"|"tribulation"|"travel"|"tragedy"|"fighting"|"adventure"|"ambient"|"boss-fight"|"tension"|"sad"|"mystical"|"excitement"|"tired"|"horror", "region": "chinese"|"japanese"|"western" (optional), "intensity": number (optional, 0 to 1) }.

ATMOSPHERIC AUDIO METADATA: When a block's scene clearly has a dominant looping background, you may set "atmosphereCategory" to exactly one of "wind", "crowd", "waves", "rain", "combat", or "noise". Optional "environment" and "atmosphereTags" arrays can add concrete, audible scene descriptors that a curated catalog can match. A busy marketplace could use category "crowd" with tags such as "market", "busy", and "chatter"; a cyberpunk factory could use "noise" with tags such as "machinery", "electrical-hum", and "industrial". These examples are not a fixed vocabulary or variation list. Never emit an asset name or ID. Omit "atmosphereCategory" when the scene has no clear fit, and never infer rain merely because characters are travelling.
...
{"type": "dialogue", "text": "\\"Who dares disturb my slumber?\\" Overseer Chen bellowed.", "metadata": {"mode": "dialogue", "speakerName": "Overseer Chen", "speakerRole": "villain", "emotion": "cruel", "intensity": 0.85, "tension": 0.9}}
{"type": "paragraph", "text": "Suddenly, the sky tore open. The Thunder Roc emerged, completely blotting out the moon.", "metadata": {"mode": "narration", "tension": 0.9, "beastEvent": {"type": "reveal", "profile": {"size": "giant", "bodyType": "bird", "element": "lightning", "movement": "flying", "intelligence": "ancient", "threatTier": "mythic", "signatureSound": "screech"}}}}
